...
const DEFAULT_WORDS = ["AI Operating System", "AetherOS"];
...
export function ParticleTextEffect({
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const fontSize = isMobile
      ? Math.min(canvas.width * 0.15, 60)
      : Math.min(canvas.width * 0.12, 100);
  ...
  const animate = () => {
    ...
    const transitionFrames = isMobile ? 300 : 240;
    frameCountRef.current++;
    if (frameCountRef.current % transitionFrames === 0) {
